...
/* ════════════════════════════════════
   ...
   Requires this in your HTML where the navbar should appear:
   <div id="navbar"></div>
════════════════════════════════════ */
async function loadSharedNavbar() {
  ...
  try {
    // Use /nav.html when nav.html is in the ROOT folder of your site.
...
function buildTabs() {
  ...
  // Prevent duplicate tabs if this script is reloaded.
  wrap.innerHTML = '';
...
function updateStickyNavHeight() {
  // Works with the new shared nav.html navbar and your older .site-nav navbar.
  const nav = document.querySelector('#navbar .navbar') || document.querySelector('.navbar') || document.querySelector('.site-nav');
  if (!nav) return;
...
function buildAdminEmail(fname, lname, email, notes, orderRows, grandTotal, grandCost) {
  ...
  const sections = orderRows.map(({name, items, subtotal, price}) => {
    ...
    const rows = items.map(({size,qty}) => `
      <tr>
        <td style="padding:8px 14px;border-bottom:1px solid #ddd;">${size}</td>
    ...
    return `
      <div style="margin-bottom:18px;border:1px solid #ddd;border-radius:6px;overflow:hidden;">
        <div style="background:#fff;padding:10px 14px;border-bottom:2px solid #d6a837;display:flex;justify-content:space-between;">
          <strong style="color:#111;font-size:13px;text-transform:uppercase;">${name}</strong>
  ...
  const notesRow = notes ? `<tr><td style="color:#666;padding:6px 0;">Name for Back</td><td>${notes}</td></tr>` : '';
...
<tr><td style="padding:22px;">
  <table width="100%">
    ...
    <tr><td style="color:#666;">Email</td><td><a href="mailto:${email}" style="color:#d6a837;">${email}</a></td></tr>
...
function buildCustomerEmail(fname, orderRows, grandTotal, grandCost) {
  const items = orderRows.map(({name,items,subtotal,price}) => `
    <div style="margin-bottom:14px;">
      <strong>${name}</strong> — ${subtotal} items · <span style="color:#d6a837;">$${subtotal*price}</span>
      <ul>${items.map(({size,qty})=>`<li>${size} ×${qty}</li>`).join('')}</ul>
    </div>`).join('');
  return `<html><body style="font-family:Arial;padding:20px;">
<h2 style="color:#d6a837;">Warrior Classic Order Confirmation</h2>
<p>Hi ${fname}, thanks for your Warrior Classic order!</p>
${items}
<hr/>
...
async function submitOrder() {
  ...
  if (!fname || !lname || !email) { showToast('Please fill in your name and email first.', true); return; }
  ...
  try {
    ...
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      subject: `New Warrior Classic Order — ${fname} ${lname}`,
...
/* ════════════════════════════════════
   INIT
════════════════════════════════════ */
async function initWarriorClassicPage() {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initWarriorClassicPage);
} else {
  initWarriorClassicPage();
}

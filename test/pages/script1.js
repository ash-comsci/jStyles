...
// ════════════════════════════════════════════════════
// ORDER FORM DATA
// Change product names, prices, and size groups here.
...
function renderProducts() {
  ...
  wrap.innerHTML = PRODUCTS.map((product, index) => {
    ...
    return `
      <article class="product-card" id="${product.id}" data-product-id="${product.id}">
        <button class="product-header" type="button" aria-expanded="true">
        ...
        <div class="product-body">${groupsHTML}</div>
      </article>
    `;
  }).join('');

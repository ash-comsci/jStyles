...
// ════════════════════════════════════════════════════
const GOOGLE_SHEETS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbz0dugfZiPeoInzkRXSKz8SECvcGN46ls5zVZaBVTcLrO8LVB2_ecJd54lxZYxoHMea9Q/exec';
const ORDER_PAGE_NAME = 'WOS Merch Order';
...
function renderModalOrderReview() {
  const summary = document.getElementById('modalOrderSummary');
  const countEl = document.getElementById('modalReviewCount');
  const costEl = document.getElementById('modalReviewCost');
  if (!summary) return;

  const { orderRows, grandTotal, grandTotalCost } = collectOrderRows();

  if (countEl) countEl.textContent = grandTotal;
  if (costEl) costEl.textContent = money(grandTotalCost);

  if (grandTotal === 0) {
    summary.innerHTML = '<p class="modal-empty-order">No items selected yet. Close this window and add at least one item before sending.</p>';
    return;
  }

  summary.innerHTML = orderRows.map(row => {
    ...
    return `
      <article class="review-product-row">
        <div class="review-product-top">
          ...
          <span>${row.priceLabel} each • ${money(row.sectionTotal)}</span>
        </div>
        <ul>${itemsHTML}</ul>
      </article>
    `;
...
function updateCustomerSummary() {
  ...
  if (!customerInfo.completed) {
    summary.textContent = 'Customer info is added after you finish selecting merch.';
    return;
  }
...
function saveCustomerInfoFromModal() {
  ...
  if (!fname || !lname || !email) {
    showToast('Please enter your first name, last name, and email before sending.', true);
    return false;
  }
...
function initCustomerModal() {
  ...
  const closeButton = document.getElementById('closeCustomerModalBtn');
...
function createOrderId() {
  ...
  return `WOS-${timestamp}-${randomPart}`;
}

function isGoogleSheetLoggingEnabled() {
  return GOOGLE_SHEETS_WEB_APP_URL &&
    !GOOGLE_SHEETS_WEB_APP_URL.includes('PASTE_YOUR_GOOGLE_APPS_SCRIPT');
}

function buildGoogleSheetPayload(orderId, fname, lname, email, notes, orderRows, grandTotal, grandTotalCost) {
  const items = orderRows.flatMap(row => row.items.map(item => ({
    productName: row.name,
    size: item.size,
    qty: item.qty,
    unitPrice: row.price,
    subtotal: item.qty * row.price
  })));

  return {
    orderId,
    submittedAt: new Date().toISOString(),
    orderPage: ORDER_PAGE_NAME,
    customer: {
      firstName: fname,
      lastName: lname,
      email,
    ...
    totals: {
      count: grandTotal,
      merchandiseTotal: grandTotalCost,
      total: grandTotalCost
    }
  };
}

async function saveOrderToGoogleSheet(payload) {
  if (!isGoogleSheetLoggingEnabled()) {
    console.warn('Google Sheet logging is not enabled yet. Paste the deployed Apps Script /exec URL into GOOGLE_SHEETS_WEB_APP_URL.');
    return { skipped: true };
  }

  // Apps Script web apps do not return a CORS-readable response to GitHub Pages.
  // "no-cors" still sends the order safely as a plain-text JSON POST.
...
function submitOrder() {
  const { grandTotal } = collectOrderRows();
...
async function submitConfirmedOrder() {
  ...
  const buttons = [
    ...
    document.getElementById('customerInfoSubmit')
  ].filter(Boolean);
  ...
  try {
    ...
    const sheetPayload = buildGoogleSheetPayload(
      orderId,
      fname,
      lname,
      email,
    ...
    let sheetLoggingFailed = false;
    let sheetLoggingNotConfigured = false;

    try {
      const sheetResult = await saveOrderToGoogleSheet(sheetPayload);
      sheetLoggingNotConfigured = Boolean(sheetResult && sheetResult.skipped);
    } catch (sheetError) {
      sheetLoggingFailed = true;
      console.error('Google Sheet logging error:', sheetError);
    }

    showToast(
      sheetLoggingFailed
        ? '✓ Order email sent, but Google Sheet logging failed. Please check the web-app URL.'
        : sheetLoggingNotConfigured
          ? '✓ Order email sent. Add the Apps Script /exec URL to enable Sheet logging.'
          : '✓ Order sent and saved to Google Sheet!'
    );
    ...
    const confirmBtn = document.getElementById('customerInfoSubmit');
    ...
    if (sidebarBtn) {
      ...
      sidebarBtn.textContent = 'REVIEW ORDER →';
    }
    if (confirmBtn) {
      confirmBtn.disabled = false;
      confirmBtn.textContent = 'CONFIRM & SEND ORDER →';
    }
  }
}
